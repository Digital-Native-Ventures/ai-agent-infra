...
if [[ -z "$ISSUE_ID" ]]; then
  echo "❌ No issue ID found in task: $TASK"
  exit 1
fi
...
# 4️⃣ Extract files and write to working tree
...
if ! jq -e '.files' "$TMPDIR/files.json" >/dev/null 2>&1; then
  echo "❌ Invalid response format from Claude:"
